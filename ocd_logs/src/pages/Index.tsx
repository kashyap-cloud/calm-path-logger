...
type ActiveTracker = null | "moment" | "insights";
type NavTab = "home" | "community" | "therapist" | "profile";

const Index = () => {
  const [activeTracker, setActiveTracker] = useState<ActiveTracker>("moment");
  const [activeTab, setActiveTab] = useState<NavTab>("home");

  React.useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const tracker = params.get("tracker");
    if (tracker === "moment" || tracker === "insights") {
            ...
            <div className="gradient-hero pt-12 sm:pt-16 pb-8 sm:pb-10 px-5 sm:px-8 rounded-b-[2rem]">
              <ScreenTransition>
                <div className="text-center text-white">
                  <h1 className="text-2xl sm:text-3xl font-bold mb-1">OCD Logs</h1>
                  <p className="text-white/80 text-sm sm:text-base">Log OCD moments and gain response insights</p>
                </div>
              </ScreenTransition>
            </div>
